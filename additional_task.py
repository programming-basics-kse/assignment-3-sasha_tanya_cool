...
def top_players(file, top_n, gender, age_kategor=None, weight_kategor=None, height_kategor=None):
    ...
    with open(file, "r", encoding='UTF-8') as file:
        ...
        for line in file:
            ...
            if weight_kategor and (weight is None or not (
                    weight_ranges[weight_kategor][0] <= weight < weight_ranges[weight_kategor][1])):
                continue

            if height_kategor and (height is None or not (
                    height_ranges[height_kategor][0] <= height < height_ranges[height_kategor][1])):
                continue
